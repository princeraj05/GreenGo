  ...
  return (
    <div className="max-w-6xl mx-auto w-full pb-10">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {cart.length > 0 && (
          <Button variant="ghost" onClick={() => navigate('/user/menu')} className="bg-white border border-slate-200">
      ...
      {cart.length === 0 ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          <Card className="p-16 text-center flex flex-col items-center border-slate-100 rounded-[2.5rem]">
            <div className="w-32 h-32 bg-slate-50 text-slate-300 rounded-full flex items-center justify-center mb-8">
              <ShoppingBag size={64} />
            </div>
            <h2 className="text-3xl font-black text-slate-900 mb-4">Your cart is feeling light</h2>
            <p className="text-slate-500 mb-8 max-w-sm text-lg font-medium">Looks like you haven't added anything to your cart yet. Let's get some food!</p>
                ...
                >
                  <Card hover className="p-4 md:p-6 flex flex-col sm:flex-row items-center gap-6 group border-slate-100">
                    ...
                    <div className="flex-1 text-center sm:text-left w-full">
                      <h3 className="text-xl font-bold text-slate-900 mb-1 group-hover:text-brand-600 transition-colors">{item.name}</h3>
                      <p className="text-slate-900 font-black text-xl mb-4">₹{item.price}</p>
                      
                      <div className="flex items-center justify-center sm:justify-start gap-4">
                        <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl p-1 shadow-sm">
                          <button onClick={() => updateQty(item._id, "dec")} className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white hover:shadow-sm text-slate-700 font-bold transition-all"><Minus size={16} /></button>
                          <span className="w-12 text-center font-bold text-slate-900">{item.qty}</span>
                          <button onClick={() => updateQty(item._id, "inc")} className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white hover:shadow-sm text-slate-700 font-bold transition-all"><Plus size={16} /></button>
                        </div>
                      </div>
                    </div>

                    <div className="w-full sm:w-auto flex justify-center sm:justify-end">
                      <button onClick={() => removeItem(item._id)} className="w-12 h-12 flex items-center justify-center text-red-500 hover:text-white bg-red-50 hover:bg-red-500 rounded-2xl transition-all shadow-sm" title="Remove">
                        <Trash2 size={20} />
                      </button>
                    </div>
                  </Card>
          ...
          <div className="w-full lg:w-[420px] shrink-0 sticky top-24 h-fit">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              ...
              <Card className="p-6 mb-6 border-slate-100">
                <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><Ticket size={20} className="text-brand-500" /> Have a Promo Code?</h3>
                <div className="flex gap-2">
                  <Input 
                    ...
                    className="font-bold tracking-widest bg-slate-50"
                  />
              ...
              <Card className="p-8 relative overflow-hidden border-slate-100 rounded-[2rem]">
                ...
                <div className="space-y-4 mb-6 text-slate-500 font-medium border-b border-slate-100 pb-6">
                  <div className="flex justify-between items-center">
                    <span>Subtotal ({cart.length} items)</span>
                    <span className="text-slate-900 font-bold">₹{subtotal.toFixed(2)}</span>
                  </div>
                  {discount > 0 && (
                    <div className="flex justify-between items-center text-emerald-600 bg-emerald-50 p-3 rounded-xl -mx-3 px-3 border border-emerald-100/50">
