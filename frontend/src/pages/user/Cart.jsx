    ...
    <div className="max-w-6xl mx-auto w-full animate-fade-in pb-10">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
          <span className="text-4xl">🛒</span> My Cart
        </h1>
      ...
      {cart.length === 0 ? (
        <div className="bg-white rounded-3xl p-16 text-center border border-slate-100 shadow-sm flex flex-col items-center">
          ...
          <button 
            ...
            className="px-8 py-4 bg-orange-500 text-white font-bold rounded-xl shadow-lg shadow-orange-500/30 hover:bg-orange-600 transition-all active:scale-95 flex items-center gap-2"
          >
            Browse Menu <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
          </button>
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-4">
            {cart.map((item) => (
              <div key={item._id} className="bg-white rounded-3xl p-4 md:p-6 flex flex-col sm:flex-row items-center gap-6 border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all group">
                <img
                ...
                <div className="flex-1 text-center sm:text-left w-full">
                  ...
                  <p className="text-orange-500 font-black text-lg mb-4">₹{item.price}</p>
          ...
          <div className="w-full lg:w-[400px] shrink-0">
            {/* Promo Code Box */}
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-6">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  placeholder="e.g. BYTEBITE20" 
                  className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-orange-500 uppercase font-bold text-slate-700 placeholder-slate-400"
                />
                <button onClick={applyPromo} className="px-6 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-colors">Apply</button>
              </div>
            </div>

            {/* Receipt UI */}
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden">
              ...
              <div className="space-y-4 mb-6 text-slate-600 font-medium">
                ...
                {discount > 0 && (
                  <div className="flex justify-between items-center text-emerald-600 bg-emerald-50 p-2 rounded-lg -mx-2 px-2">
                    <span className="flex items-center gap-1"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 11l5-5m0 0l5 5m-5-5v12" /></svg> Discount (20%)</span>
              ...
              <div className="flex justify-between items-center pt-6 border-t-2 border-dashed border-slate-200 mb-8">
                <span className="text-xl font-bold text-slate-900">Total To Pay</span>
                <span className="text-4xl font-black text-orange-500">₹{total.toFixed(0)}</span>
              </div>

              <button
                onClick={() => navigate("/user/checkout")}
                className="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold text-lg shadow-lg shadow-orange-500/30 transition-all active:scale-95 flex items-center justify-center gap-2"
              >
                Checkout Securely
