...
export default function About() {
  ...
  return (
    <div className="w-full flex flex-col bg-white overflow-hidden">
      ...
      {/* ── Hero Section ── */}
      <section className="relative w-full min-h-[500px] flex items-center justify-center px-4 py-20 bg-gray-50 overflow-hidden">
            ...
            <div className="flex flex-wrap justify-center gap-8 md:gap-16 pt-8 border-t border-gray-200">
              {[
                ...
                <div key={i} className="flex flex-col items-center">
                  <span className="text-3xl font-black text-orange-600">{stat.val}</span>
                  <span className="text-sm font-semibold text-gray-500 uppercase tracking-widest mt-1">{stat.label}</span>
                </div>
              ))}
            </div>
          </Section>
      ...
      <section className="w-full py-24 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          <Section>
            <div className="bg-orange-50 rounded-[2.5rem] p-10 md:p-16 text-center border border-orange-100 shadow-xl shadow-orange-100/50 relative overflow-hidden">
      ...
      <section className="w-full py-24 bg-gray-50 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <Section className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="text-sm font-bold text-orange-600 uppercase tracking-widest mb-3">What We Stand For</h2>
            <h3 className="text-4xl font-black text-gray-900 tracking-tight">Our Core Values</h3>
          </Section>
      ...
      {/* ── Timeline ── */}
      <section className="w-full py-24 px-4 sm:px-6 bg-white">
        <div className="max-w-4xl mx-auto">
          <Section className="text-center mb-16">
            <h2 className="text-sm font-bold text-orange-600 uppercase tracking-widest mb-3">Our Journey</h2>
            <h3 className="text-4xl font-black text-gray-900 tracking-tight">Growing Year by Year</h3>
          </Section>

          <div className="flex flex-col gap-8 relative before:absolute before:inset-0 before:ml-[3.5rem] md:before:mx-auto md:before:translate-x-0 before:h-full before:w-1 before:bg-gray-100">
            {milestones.map((m, i) => (
              <Section delay={i * 100} key={m.year} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                {/* Timeline Dot */}
                <div className="flex items-center justify-center w-8 h-8 rounded-full border-4 border-white bg-orange-500 absolute left-[3.5rem] md:left-1/2 -translate-x-1/2 shadow-md group-hover:scale-125 transition-transform z-10"></div>
                
                {/* Year Label for Mobile (hidden on desktop) */}
                <div className="md:hidden w-[3rem] shrink-0 text-right pr-4 font-black text-orange-600">{m.year}</div>

                {/* Card */}
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2rem)] bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-lg transition-shadow">
                  <div className="hidden md:block font-black text-orange-600 text-lg mb-2">{m.year}</div>
                  <h4 className="text-xl font-bold text-gray-900 mb-2">{m.title}</h4>
                  <p className="text-gray-500 font-medium">{m.desc}</p>
                </div>
              </Section>
            ))}
          </div>
      ...
      <section className="w-full py-24 bg-gray-50 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <Section className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="text-sm font-bold text-orange-600 uppercase tracking-widest mb-3">Meet the Team</h2>
            <h3 className="text-4xl font-black text-gray-900 tracking-tight">The People Behind ByteBite</h3>
          </Section>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {team.map((t, i) => (
              <Section delay={i * 150} key={t.name}>
                <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm text-center hover:-translate-y-2 transition-transform duration-300">
                  <div className="w-24 h-24 mx-auto bg-orange-50 rounded-full flex items-center justify-center text-5xl mb-6 shadow-inner">
                    {t.emoji}
                  </div>
                  <h4 className="text-xl font-bold text-gray-900 mb-1">{t.name}</h4>
                  <p className="text-orange-600 font-bold mb-3">{t.role}</p>
                  <p className="text-gray-400 font-medium text-sm">📍 {t.city}</p>
                </div>
              </Section>
            ))}
          </div>
