  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1">Track assignments, delivery progress, and COD credit.</p>
      </div>

      {loading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
          {[1, 2, 3, 4].map((item) => <div key={item} className={`${cardClass} h-32 animate-pulse bg-slate-100 dark:bg-slate-900`} />)}
        </div>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
